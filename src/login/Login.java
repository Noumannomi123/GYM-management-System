...
import gymClasses.User;
import mainwindows.AdminW;
import mainwindows.MemberW;
import mainwindows.RegistrationManagerW;
import mainwindows.StaffW;
import mainwindows.UserW;
        ...
        btnNewButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (username.isEmpty()) {
                	...
                	User user = new User();
                    if (user.login(username, uType, password)) {
                        // Successful login, close current JFrame
                        dispose();
                        // Store the data into the attributes
                        UserW newFrame;
                        if(uType == "Member")
                        	newFrame = new MemberW(database);
                        else if(uType == "Staff")
                        	newFrame = new StaffW(database);
                        else if(uType == "Admin")
                        	newFrame = new AdminW(database);
                        else
                        	newFrame = new RegistrationManagerW(database);
